...
REAL orient2d(REAL* pa, REAL* pb, REAL* pc)
{
  ...
  //printf("Determinant: %f - detleft: %f, detright: %f\n", det, detleft, detright);

  if (detleft > 0.0) {
    if (detright <= 0.0) {
      //printf("REtornou por aqui 0\n");
      return det;
    } else {
    ...
    if (detright >= 0.0) {
      //printf("REtornou por aqui 1\n");
      return det;
    } else {
      detsum = -detleft - detright;
    }
  } else {
    //printf("REtornou por aqui 2\n");
    return det;
  }

  errbound = ccwerrboundA * detsum;
  //printf("Errbound: %f - ccwerrboundA: %f\n", errbound, ccwerrboundA);
  if ((det >= errbound) || (-det >= errbound)) {
    //printf("REtornou por aqui 3\n");
    return det;
  }
